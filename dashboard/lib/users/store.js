...
exports.claimAdminIfNoneExists = claimAdminIfNoneExists;
exports.generateTemporaryPassword = generateTemporaryPassword;
exports.createUser = createUser;
exports.resetPassword = resetPassword;
exports.setOwnPassword = setOwnPassword;
...
async function listUsers() {
    const rows = await (0, db_1.sql)(`SELECT ${COLUMNS} FROM app_users ORDER BY role, LOWER(email)`);
    return rows.map(toUser);
}
async function findUserByEmail(email) {
    const rows = await (0, db_1.sql)(`SELECT ${COLUMNS} FROM app_users WHERE LOWER(email) = LOWER($1)`, [email]);
    return rows[0] ? toUser(rows[0]) : null;
}
async function countUsers() {
    const rows = await (0, db_1.sql)(`SELECT COUNT(*)::text AS n FROM app_users`);
 ...
 */
async function claimAdminIfNoneExists(email) {
    const rows = await (0, db_1.sql)(`INSERT INTO app_users (email, role, must_change_password)
...
function generateTemporaryPassword() {
    const words = Array.from({ length: 3 }, () => WORDS[(0, crypto_1.randomInt)(WORDS.length)]);
    return `${words.join("-")}-${(0, crypto_1.randomInt)(1000, 10000)}`;
}
...
async function createUser(input) {
    ...
    const rows = await (0, db_1.sql)(`INSERT INTO app_users (email, name, role, client_id, password_hash, must_change_password)
...
async function resetPassword(id) {
    const temporaryPassword = generateTemporaryPassword();
    await (0, db_1.sql)(`UPDATE app_users
        SET password_hash = $2, must_change_password = TRUE, updated_at = NOW()
      WHERE id = $1::bigint`, [id, await hash(temporaryPassword)]);
    return temporaryPassword;
}
/** The user choosing their own password — clears the forced-change flag. */
async function setOwnPassword(email, password) {
    await (0, db_1.sql)(`UPDATE app_users
        SET password_hash = $2, must_change_password = FALSE, updated_at = NOW()
      WHERE LOWER(email) = LOWER($1)`, [email, await hash(password)]);
}
async function updateUser(id, input) {
    await (0, db_1.sql)(`UPDATE app_users
        SET role = $2, client_id = $3, is_active = $4, updated_at = NOW()
      WHERE id = $1::bigint`, [id, input.role, input.role === "client" ? input.clientId : null, input.isActive]);
}
async function deleteUser(id) {
    await (0, db_1.sql)(`DELETE FROM app_users WHERE id = $1::bigint`, [id]);
}
/**
...
async function verifyPassword(email, password) {
    const rows = await (0, db_1.sql)(`SELECT password_hash, is_active FROM app_users WHERE LOWER(email) = LOWER($1)`, [email]);
    const row = rows[0];
    if (!row?.password_hash || !row.is_active) {
        ...
        await bcryptjs_1.default.compare(password, "$2a$12$" + "x".repeat(53));
        return null;
    }
    if (!(await bcryptjs_1.default.compare(password, row.password_hash)))
        return null;
    await (0, db_1.sql)(`UPDATE app_users SET last_login_at = NOW() WHERE LOWER(email) = LOWER($1)`, [email]);
